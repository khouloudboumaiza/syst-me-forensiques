...
Moteur de classification autonome des IOCs (hash + fichiers), basé sur :
  ...
  - une couche IA optionnelle via Ollama si elle est disponible localement
...
def _call_gemini(prompt: str, model: str = "gemini-2.5-flash") -> Optional[str]:
    """Appelle l'API Gemini si la bibliothèque et la clé sont disponibles."""
    if genai is None:
        return None
...
def classify_by_vt_score_with_ai(
    hash_value: str = "",
    file_path: str = "",
    vt_malicious: int = 0,
    vt_total: int = 0,
    ...
    """Même logique que classify_by_vt_score, avec un enrichissement optionnel via Gemini."""
    ...
    prompt = (
        f"Tu es un analyste SOC. Classe ce hash comme un IOC de type: "
        f"true_positive, likely_false_positive, potential_false_negative, suspicious_review ou clean. "
        f"Base-toi sur le score VirusTotal fourni et sur le chemin du fichier. "
        f"Hash: {hash_value or 'inconnu'}\n"
        f"Chemin: {file_path or 'non renseigné'}\n"
        f"VirusTotal: {vt_malicious}/{vt_total} malveillant(s) sur {vt_total} moteur(s)\n"
        f"Verdict VT: {vt_verdict}\n"
        f"Statut déterministe à justifier: {result['status']}\n"
        f"Réponds uniquement avec un JSON valide: {{\"status\": \"...\", \"explanation\": \"...\", \"recommendation\": \"...\"}}"
    )

    ai_text = _call_gemini(prompt, model=model)
    if ai_text:
        try:
            ...
            if isinstance(parsed, dict):
                ...
                if isinstance(parsed.get("explanation"), str) and parsed.get("explanation"):
                    result["explanation"] = parsed["explanation"]
                if isinstance(parsed.get("recommendation"), str) and parsed.get("recommendation"):
