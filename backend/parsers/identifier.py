...
def detect_tool(filename: str, content: bytes) -> str:
    """Retourne 'loki' | 'hayabusa' | 'ml-network' | 'autopsy' | 'kuiper' | 'zircolite' | 'unknown'"""
    name = filename.lower()
    try:
    ...
    if name.endswith(".json"):
        try:
            ...
            if isinstance(data, list) and data and isinstance(data[0], dict):
                ...
                # Clés typiques Zircolite JSON
                if {"sigma", "matches", "title"} & keys or {"rule", "matches"} & keys:
                    return "zircolite"
        except Exception:
            pass
